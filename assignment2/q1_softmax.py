...
def softmax(x):
    ...
    ### YOUR CODE HERE
    maximum = tf.reduce_max(x, 1, True)
    sumexpx = tf.exp(x-maximum)
    out = tf.divide(sumexpx, tf.reduce_sum(sumexpx,1,True))
    ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
    ...
    ### YOUR CODE HERE
    float32y = tf.to_float(y)
    log_yhat = tf.log(yhat)
    ynode = tf.multiply(float32y, log_yhat)
    out = -tf.reduce_sum(ynode)
