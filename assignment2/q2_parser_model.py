...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        tensor = tf.Variable(self.pretrained_embeddings)
        # tf.nn.embedding_lookup(params, ids, partition_strategy='mod', name=None, validate_indices=True, max_norm=None)
        words = tf.nn.embedding_lookup(tensor, self.input_placeholder)
        # [-1] flattens into 1-D
        embeddings = tf.reshape(words, [-1, self.config.n_features*self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        W = tf.Variable(xwi((self.config.n_features*self.config.embed_size, self.config.hidden_size)),dtype = tf.float32)
        b1 = tf.Variable(xwi((self.config.hidden_size,)),dtype = tf.float32)
        U = tf.Variable(xwi((self.config.hidden_size,self.config.n_classes)),dtype = tf.float32)
        b2 = tf.Variable(xwi((self.config.n_classes,)),dtype = tf.float32)
        # h = Relu(xW + b1)
        # h_drop = Dropout(h, dropout_rate)
        # pred = h_dropU + b2
